...
const {Theaters,Screens,Seats} = require('../models/dbhelper');
      ...
      [
        ...
        if(screen_of_theater){
         ...
         if(screen_of_theater.total_seats < silver+gold+platinum){
            res.status(202).send('to many seats');
         }else{
        ...
        res.status(200).send({ message: "Seat created successfully!", screen_of_theater });
    } catch (err) {
