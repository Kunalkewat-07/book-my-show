...
exports.createSeat = async (req, res) => {
  try {
    ...
    let { silver, gold, platinum, platinum_price, silver_price ,gold_price} = req.body;
    ...
    if (req.user.role == "user" || req.user.role == 'admin') {
      ...
      const is_seat_available = await Seats.findOne({where:{screen_id}});
      if(is_seat_available){
        return res.status(401).json("all seats are full")
      }
    ...
    if (screen_of_theater) {
      ...
      if (screen_of_theater.total_seats < silver + gold + platinum) {
        return res.status(202).send("we dont'have enough seats");
      } else {
